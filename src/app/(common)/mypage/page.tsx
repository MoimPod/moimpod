  ...
  await Promise.all([
    ...
    queryClient.prefetchQuery({
      ...
      queryFn: async () => {
        ...
        const response = await axios.get<ReviewsResponse>(`${process.env.NEXT_PUBLIC_API_BASE_URL}reviews`, {
          params: {
            limit: 100,
            userId: user?.id,
          },
    ...
    queryClient.prefetchQuery({
      ...
      queryFn: async () => {
        const user = queryClient.getQueryData<UserType>(["user"]);
        const response = await axios.get<GatheringType[]>(`${process.env.NEXT_PUBLIC_API_BASE_URL}gatherings`, {
          params: {
            limit: 100,
            sortBy: "dateTime",
            sortOrder: "desc",
